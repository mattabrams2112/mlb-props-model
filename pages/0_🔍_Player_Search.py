"""Player Search — manual batter lookup with model projection and rating breakdown."""
import sys, os
...
st.set_page_config(page_title="Player Search | MLB Props", page_icon="🔍", layout="wide")
inject_styles()
...
st.markdown(f"## 🔍 Player Search — {datetime.now().strftime('%B %d, %Y')}")
...
with col_title:
    st.markdown("### Today's Batters — Sorted by Rating")
with col_refresh:
    if st.button('🔄 Refresh Lineups', use_container_width=True,
                 help='Reloads all lineups — use this after games start'):
...
if not games:
    ...
    if 'lineup_rows' in st.session_state and st.session_state['lineup_rows']:
        ...
        st.caption(f'{len(rows)} batters · highest to lowest rating · click Refresh to update')
