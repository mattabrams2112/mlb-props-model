"""
Game View — both lineups side by side with HRR ratings per batter.
Faster: no CV, fewer trees. Each batter loads and renders immediately.
"""
import sys, os
...
def render_lineup(container, batter_ids, is_home, opp_pitcher_id, park_team,
    ...
    for idx, pid, pname, pteam, res in fetched:
        ...
        if res:
            ...
            row = (f'<tr style="background:{bg};">'
                   f'<td style="padding:6px 8px;color:#475569;">{batting_order}</td>'
                   f'<td>{logo_img_tag(pteam,20)}</td>'
                   f'<td style="color:#e0f2fe;padding:6px 8px;">{pname}'
    ...
    if totals:
        ...
        rc    = c(avg_r, 75, 55)
        pc    = c(tot_p / max(len(totals), 1), 3.0, 2.0)
        totals_row = (
    ...
    for i, g in enumerate(games):
        with cols[i % 4]:
            ...
            st.markdown(
                ...
                f'<span style="font-size:11px;color:#94a3b8;">{w_txt}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
    st.stop()
...
for game in games:
    ...
    with ac:
        ...
        if ab_ids:
            render_lineup(ac, ab_ids, False, home_pid, home,
                          weather, game_label, home_p)
        else:
    ...
    with hc:
        ...
        if hb_ids:
            render_lineup(hc, hb_ids, True, away_pid, home,
                          weather, game_label, away_p)
        else:
            st.info('Lineup pending.')
